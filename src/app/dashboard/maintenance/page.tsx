...
import { Wrench, AlertTriangle, Hammer, CheckCircle2, Plus } from "lucide-react";
import { TicketStatusDropdown } from "./MaintenanceClientComponents";
...
export default async function MaintenancePage() {
  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      {openTickets > 0 && (
        <div className="mb-6 bg-white p-6 rounded-xl shadow-sm border border-orange-200 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className="p-3 bg-orange-50 rounded-full">
              <AlertTriangle className="w-6 h-6 text-orange-600" />
            </div>
            <div>
              <h3 className="text-lg font-bold text-gray-900">Open Issues</h3>
              <p className="text-gray-500">There are {openTickets} unresolved maintenance tickets.</p>
            </div>
          </div>
        </div>
      )}
            ...
            tickets.map(ticket => (
              <div key={ticket.id} className="p-6 flex items-start justify-between hover:bg-gray-50 transition-colors">
                <div>
                  ...
                  <div className="text-sm text-gray-600 mt-2 bg-gray-50 p-3 rounded-lg border border-gray-200 prose prose-sm max-w-none prose-p:my-0">
