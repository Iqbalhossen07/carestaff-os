...
export default function EditShiftFormClient({ shift, staffMembers }: { shift: any, staffMembers: any[] }) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await updateShift(shift.id, data);
      ...
      Swal.fire('Error', error.message || "Failed to update shift", 'error');
      setLoading(false);
    }
  };
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
        <div>
          ...
          <input type="datetime-local" name="startTime" defaultValue={toLocalISO(shift.startTime)} required className="w-full px-4 py-2.5 bg-gray-50 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none" />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">End Time *</label>
          <input type="datetime-local" name="endTime" defaultValue={toLocalISO(shift.endTime)} required className="w-full px-4 py-2.5 bg-gray-50 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none" />
        </div>
      </div>
      ...
      <div>
        ...
        <textarea name="notes" defaultValue={shift.notes || ""} rows={3} placeholder="Any specific tasks or instructions for this shift..." className="w-full px-4 py-2.5 bg-gray-50 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none resize-none"></textarea>
      </div>
