...
function CarerTopbar({ dbUser }: { dbUser?: any }) {
      ...
      <div className="relative" ref={dropdownRef}>
        ...
        >
          <div className="text-right hidden md:block">
            ...
            <p className="text-xs text-blue-600 font-bold">Care Staff</p>
          </div>
...
export default function CarerLayoutClient({ children, dbUser }: { children: React.ReactNode, dbUser?: any }) {
    ...
    <div className="flex h-screen bg-gray-50 text-gray-900 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <nav className="fixed bottom-0 left-0 w-full bg-white border-t border-gray-200 lg:hidden flex justify-around items-center h-16 z-30 pb-safe shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
          ...
          <Link href="/carer" className={`flex flex-col items-center justify-center w-full h-full transition-colors ${pathname === '/carer' ? 'text-blue-600' : 'text-gray-500 hover:text-blue-600'}`}>
            <LayoutDashboard className="w-5 h-5 mb-1" />
            <span className="text-[10px] font-bold">Home</span>
          </Link>

          <Link href="/carer/residents" className={`flex flex-col items-center justify-center w-full h-full transition-colors ${pathname.startsWith('/carer/residents') ? 'text-blue-600' : 'text-gray-500 hover:text-blue-600'}`}>
            <HeartPulse className="w-5 h-5 mb-1" />
            <span className="text-[10px] font-bold">Residents</span>
          </Link>

          <Link href="/carer/emar" className={`flex flex-col items-center justify-center w-full h-full transition-colors ${pathname.startsWith('/carer/emar') ? 'text-blue-600' : 'text-gray-500 hover:text-blue-600'}`}>
